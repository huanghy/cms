...
<body class="gray-bg">
            ...
            <div class="col-sm-12">
                <div class="ibox ">
                    <div class="ibox-title">
                        <div class="affair-tit">角色管理</div>
                    </div>
                    <div class="ibox-content">
                        <p>
                        <@shiro.hasPermission name="system:role:add">
                        	<button class="btn btn-success addbtns" type="button" onclick="add();"><i class="fa fa-plus"></i>&nbsp;添加</button>
                        </@shiro.hasPermission>
                        </p>
    ...
    <script>
			...
			$("#table_list").bootstrapTable({
			    ...
			    columns: [{
                    ...
                    formatter: function (value, row, index) {
                        ...
                        operateHtml="<i class='fa fa-edit warter' title='修改' onclick=edit('"+row.id+"')></i><i class='fa fa-trash-o warter' title='删除' onclick=del('"+row.id+"')></i><i class='fa fa-chain warter' title='分配资源' onclick=grant('"+row.id+"')></i>"
        ...
        function del(id){
        	layer.confirm('确定删除当前角色吗?', {icon: 3, title:'提示'}, function(index){
        		$.ajax({
    	    		   type: "POST",
    	    		   dataType: "json",
