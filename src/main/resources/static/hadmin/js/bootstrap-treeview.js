...
;(function ($, window, document, undefined) {
	...
	Tree.prototype.subscribeEvents = function () {
		...
		//this.$element.on('contextmenu',$.proxy(this.mouseHandler, this));
	...
	*/
	...
	/***鼠标事件
	 */
	Tree.prototype.mouseHandler = function (event) {
		if (!this.options.enableLinks) event.preventDefault();
		var target = $(event.target);
		var node = this.findNode(target);
		if (!node || node.state.disabled) return;
		if (typeof (this.options.onMouseDown) === 'function') {
			this.$element.on('onMouseDown', this.options.onMouseDown(event,node));
		}
	};
	...
	Tree.prototype.clickHandler = function (event) {
		if (!this.options.enableLinks) event.preventDefault();
		var target = $(event.target);
		var offsetTop = this.$wrapper[0].scrollTop;
		var node = this.findNode(target);
		if (!node || node.state.disabled) return;
		if(typeof(target)!='undefined' 
			&& (target[0].tagName=='EM' || target[0].tagName=='em')){
			if (typeof (this.options.onMouseDown) === 'function') {
				this.options.onMouseDown(event,node);
			}
		}/**绑定其他事件***/ 
		else {
	...
	Tree.prototype.buildTree = function (nodes, level) {
		...
		$.each(nodes, function addNodes(id, node) {
			var treeItem = $(_this.template.item)
	            ...
	            if(_this.options.badges)
	            {
	            	treeItem.append(_this.template.badges);
	            }
			...
			if(_this.options.showMoreButton){
				treeItem.append($(_this.template.moreButton).addClass(classList.join(' ')));
			}
	...
	Tree.prototype.template = {
		...
		moreButton: '<em  class=\'ys fa fa-bullseye\' title="编辑" ></em>',
		badges:'<b class="bge fa fa-check"></b>'		
	};
